...
void Server::createSocket()
{
    ...
    fcntl(_serverFd, F_SETFL, O_NONBLOCK); //soketi nonblock hale getirir.
    const int enable = 1;
...
{
    std::map<std::string, std::vector<std::string> > params = getParams(str); // str boşluklardan ayrıştırılır ve her bir string string vectorunde ayrı nesne olarak tutuluyor.
...
{
    for (cliIt it = _clients.begin(); it != _clients.end(); ++it) { // _clients vektöründe yer alan her istemci için bir döngü başlatır.
        ...
        {                                        // Eğer istemcinin dosya tanımlayıcısı okuma işlemi için hazırsa, bu koşul sağlanır.
            *state = 0;
            ...
            if (readed <= 0) { // Eğer okunan veri yoksa veya bir hata oluşmuşsa
            ...
            else // Eğer veri okunduysa, bu blok çalışır.
            {
                ...
                else //eğer okunan verinin sonunda yeni satır karakteri varsa, bu veri işlenir.
...
void Server::run()
{
    ...
    while (1)
    {
        ...
        if (FD_ISSET(_serverFd, &_readFdsSup)) {      // FD_ISSET makrosu, belirtilen fd'nin set kümesinde bulunup bulunmadığını kontrol eder. 
            acceptRequest();                          // Eğer fd kümede bulunuyorsa ve belirli bir olaya hazır durumdaysa, FD_ISSET true değerini döndürür; 
            state = 0; continue;                      // aksi takdirde, false döndürür
        }
        if (state) { // hata durumu varsa state 1 olmuşsa buraya girer
            readEvent(&state);
            if (state == 0)
                continue;
        }
